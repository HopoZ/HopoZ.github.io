...
# 头文件
  ...
  ## `termios.h`
    即terminal i/o settings, 是一个C语言的头文件，用于POSIX（Portable Operating System Interface）系统上的终端I/O接口。这个头文件提供了对终端接口进行控制的API，允许程序员配置和管理终端或伪终端的行为，例如设置字符回显、行处理、控制字符等。
  如termios结构体，储存了终端属性
